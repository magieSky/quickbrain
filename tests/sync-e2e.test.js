...
describe('e2e sync round-trip', () => {
  ...
  it('client pushAllToServer → server pull → second client receives all notes', async () => {
    ...
    const id1 = addNote(clientDb, { content: 'first note', title: 'First', tags: ['a'], is_private: 0 })
    const id2 = addNote(clientDb, { content: 'second note', title: 'Second', tags: ['b'], is_private: 0 })
