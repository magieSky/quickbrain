...
async function applyOps(db, userId, ops) {
  const accepted = []
  const conflicts = []
  for (const op of ops) {
    if (op.op === 'upsert') {
      const r = await notes.upsertNote(db, userId, op.note)
      ...
      const r = await notes.softDelete(db, userId, op.client_id, op.updated_at)
...
module.exports = async function syncRoutes(fastify, opts) {
  ...
  fastify.get('/v1/sync/pull', async (req, reply) => {
    ...
    const rows = await notes.listChangedSince(db, v.userId, q.since, q.limit)
